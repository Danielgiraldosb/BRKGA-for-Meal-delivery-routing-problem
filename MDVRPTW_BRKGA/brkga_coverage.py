"""
BRKGA for MDVRPTW - Coverage Maximization
Implements Biased Random-Key Genetic Algorithm to maximize order coverage
"""
...
class BRKGACoverage:
    """
    BRKGA Solver for MDVRPTW with coverage maximization objective
    """
    ...
    def solve(self):
        ...
        for generation in range(self.params.max_generations):
            ...
            for i in range(self.params.population_size):
                solution = decode_chromosome(
                    ...
                    self.params, fitness_type='coverage'
                )
            ...
            if self.params.verbose and generation % 10 == 0:
                print(f"Gen {generation}: Best fitness = {self.best_fitness:.4f}, "
                      f"Coverage = {self.best_solution['coverage_rate']*100:.2f}%, "
        ...
        print(f"\n{'='*60}")
        print(f"BRKGA-Coverage FINISHED")
        print(f"{'='*60}")
    ...
    def save_solution(self, output_folder='results'):
        ...
        output_dir = os.path.join('MDVRPTW_BRKGA', output_folder, 'coverage')
        os.makedirs(output_dir, exist_ok=True)

        output_path = os.path.join(
            output_dir,
            f'resultadosIns_{self.instance_info["name"]}_coverage.xlsx'
        )
...
def main():
    """
    Main execution function - run BRKGA-Coverage on specified instance
    """
    import argparse

    parser = argparse.ArgumentParser(description='BRKGA for MDVRPTW - Coverage Maximization')
    ...
    print(f"\n{'='*60}")
    print(f"BRKGA-Coverage for MDVRPTW")
    print(f"{'='*60}")
